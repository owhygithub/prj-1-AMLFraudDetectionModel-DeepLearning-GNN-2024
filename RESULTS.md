...
| Repeats | 7–8 independent runs per variant |
| Decision threshold | 0.6 on the model's output score |

Each variant's numbers below are the mean ± standard deviation across its
repeats. The spread is tiny (fourth decimal place) because the repeats shared a
fixed data split and differed only in weight initialisation.
...
Hyper-parameters were fixed per decoder family after an Optuna search:
DistMult used 25 embedding channels and weight decay 5e-4; ComplEx used 20
channels and 5e-5. Both used lr 0.01, dropout 0.1, and up to 100 epochs.
...
test transactions (40,000 laundering):
...
and ROC-AUC spans 0.8751 to 0.8800. Those gaps are around 0.005 — five times
the run-to-run standard deviation, so not quite noise, but nothing that would
change a decision. Neither the choice of decoder nor the time signal moved the
needle. Given
[§ Known defects](#known-defects-in-the-original-runs), that is what you would
expect: with a zero imaginary part, ComplEx *is* DistMult, and with a detached
weight parameter, `-T+W` *is* `-T`. Three of the six "variants" were never
distinct models.

**There is a precision/recall trade to pick from.** The variants sit on a
shallow frontier: `ComplEx` recovers the most laundering (recall 0.877),
`ComplEx-T+W` raises the fewest false alarms (precision 0.850). For AML triage,
where a missed case costs more than an extra review, the recall end is the one
to take.
...
thresholded at 0.6 directly rather than as a probability — the equivalent of
demanding `P ≥ 0.65` where the others demanded `P ≥ 0.6`. That alone explains
its collapsed recall. But it also has the worst ROC-AUC (0.707 vs ~0.88), and
AUC is threshold-free, so the thresholding bug is not the whole story:
unbounded trilinear scores fed straight into the loss appear to have trained
less stably than the accidentally squashed ones. On the smaller pilot dataset,
where scores had less room to grow, the same configuration reached AUC 0.837 —
consistent with that reading, though not proof of it.
...
100,000-edge ranking, so its scale is set by the number of positives, not by
model quality. The cleaned-up code keeps the metric for continuity but adds
`average_precision`, which is the ranking number to read.

---

## Pilot round

An earlier round on a much smaller graph (2,000 test transactions), before the
...
Accuracy barely moved between the pilot and the 50× larger main run, while
...
and the observed losses sit at 0.60–0.62, just above it. It also attenuates the
gradient by up to 16× and throws away the numerically stable log-sum-exp that
is the whole point of using the fused loss.

Affects: `DisMult-T`, `DisMult-T+W`, `ComplEx`, `ComplEx-T`, `ComplEx-T+W`.
Fixed by returning logits from the decoder (`models.py`).
...
choosing for directed relations — "A paid B" ≠ "B paid A" — was never present.
Fixed by splitting each embedding into real and imaginary halves
(`complex_score` in `models.py`).
...
the time gate — which is consistent with `-T+W` scoring marginally *below*
`-T` in every case. Fixed by using the parameter directly.
...
by an unknown margin. Fixed by folding within the training split only
(`cross_validate` in `train.py`).
...
- **Full-graph message passing.** The adjacency matrix always covers every
  ...
  *exists* while learning — standard for transductive link prediction, but
  worth stating.
- **One convolution layer.** Each account only ever sees its immediate
  neighbours. Laundering patterns (layering, smurfing) span several hops.

---

## Reproducing

The numbers above cannot be reproduced from this repository as-is: the
...
Expect the absolute numbers to differ. The fixes cut both ways: removing the
CV leak should lower the scores, while a real ComplEx decoder and a working
...
1. **Normalise the score, on purpose.** The accidental sigmoid appears to have
   been doing useful work. A deliberate bound — batch norm on the score, or
   tanh — would test that directly instead of by accident.
...
4. **Report precision at a fixed alert budget.** "How many true cases in the
   top 1,000 flags" is the number an AML team actually operates on.
